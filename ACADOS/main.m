...
% ========================================================================
% The code below serves as the main simulation file. It loops over all
% cases and scenerios and computer the collision free path and then follows
% that path using NMPC. All data for every simulation is stored in 
% SimDataFinal.mat. TestPathFixed.mat is used when that file does not exist
% yet. It only contains the reference line and obstacles. When running the
% code for the first time, SimDataFinal.mat is created and the data is
% added.
...
% Define the method:
% local_MPC   - Standard NMPC following cenreline
% global_MPC  - Trajectory optimization + NMPC
% RRT         - RRT with Motion primitives + NMPC 
method = "RRT";
index.method = method;

% Load data
if exist('SimDataFinal.mat', 'file') == 2 % Data storage struct
    data = load('SimDataFinal.mat');
else
    data = load('../SETUP/TestPathFixed.mat'); %Serves as the setup file
end

% Outer loop: Loop over all cases
for icase = caseNumbers
    
    % Save index structure for use in lower files
    index.icase = icase;
    
    save index index

    % Inner loop: Loop over multiple runs for a given case
    for j = 1:10 
        ...
        elseif index.method == "global_MPC"
            % Run the reference optimization part to find optimal collision
            % free path
            ...
            % Run the MPC simulation using the previous reference results
