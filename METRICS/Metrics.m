...
dataFilePath = '../ACADOS/SimDataFinal.mat'; % Relative path to the .mat file
load(dataFilePath);
    ...
    for icase = 1:nCase
        for j = 1:nScenarios
            ...
            if isempty(endindex)
                ...
                disp(icase)
                disp(j)
                endindex = length(output);
            end

            route = output(1:endindex,3:4);%adjust to where t, x and y is in data
            ...
            if method == "local_MPC"
                ...
                scenarios(icase,j).metricsLocalMPC.time_to_goal = output(endindex,1);
                scenarios(icase,j).metricsLocalMPC.path_length = path_length;
                scenarios(icase,j).metricsLocalMPC.local_solver_time = output(1:endindex,10);
            ...
            elseif method == "global_MPC"
                ...
                scenarios(icase,j).metricsGlobalMPC.time_to_goal = output(endindex,1);
                scenarios(icase,j).metricsGlobalMPC.path_length = path_length;
                scenarios(icase,j).metricsGlobalMPC.local_solver_time = output(1:endindex,10);
                scenarios(icase,j).metricsGlobalMPC.global_solver_time = scenarios(icase,j).solstatOptimalReference.solver_time;
            ...
            elseif method == "RRT"
                ...
                scenarios(icase,j).metricsMotionPrimitives.time_to_goal = output(endindex,1);
                scenarios(icase,j).metricsMotionPrimitives.path_length = path_length;
                scenarios(icase,j).metricsMotionPrimitives.local_solver_time = output(1:endindex,10);
